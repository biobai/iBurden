...
Usage:
    case_vcf_vep.R [--vcfSource] [--maxAF] [--snvType] <input> <output>
...
Options:
    ...
    --snvType=<missense_variant> missense_variant or other "missense_variant|start_lost|stop_lost|protein_altering_variant|stop_gained" [default: missense]
Arguments:
...
if (controlVcfFileType=="ExAC") {
	...
	anCol="AN_Adj"
	nhomaltCol="AC_Hom"
	ScanVcfParamInfo=c(acCol,anCol,nhomaltCol,"DP_HIST",vepCol) #Don't need AF here as we will use adjusted AF which is AC_Adj/AN_Adj
	ScanVcfParamFixed=c("ALT","FILTER") #ALT is needed to expand the vcf to one alt allele per row
	filters <- FilterRules(list(AF=AF_max_ExAC))
	...
	anCol="AN"
	nhomaltCol="nhomalt"
	ScanVcfParamInfo=c(acCol,anCol,"AF",nhomaltCol,vepCol)
...
tab <- TabixFile(controlVcfFile, yieldSize=10000)
open(tab)
vcfProgress=0
